	...
	internal class ChartLicenseProvider : LicenseProvider
	{
		protected virtual bool IsKeyValid(string key, Type type)
		{
			if (key != null)
			{
				...
				try
				{
					ProductKeyEncoder productKeyEncoder = new ProductKeyEncoder();
					text = productKeyEncoder.DecryptLicenseKey(key);
				}
				catch
				{
				...
				if (array.Length >= 2 && array[0] == Chart.productID && array[1] == this.GetNic())
				{
					return true;
				}
				return false;
		...
		{
			return this.IsKeyValid(key, type);
		}

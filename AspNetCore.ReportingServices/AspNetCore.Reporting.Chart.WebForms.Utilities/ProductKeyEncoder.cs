...
namespace AspNetCore.Reporting.Chart.WebForms.Utilities
{
	internal class ProductKeyEncoder
	{
		...
		public string DecryptLicenseKey(string key)
		{
			...
			byte[] array = this.StringToBytes(key);
			SymmetricAlgorithm symmetricAlgorithm = SymmetricAlgorithm.Create();
			CryptoStream cryptoStream = new CryptoStream(memoryStream, symmetricAlgorithm.CreateDecryptor(this.rijnKey, this.rijnIV), CryptoStreamMode.Write);
			...
			cryptoStream.Close();
			return this.BytesToString(memoryStream.ToArray());
		}
	}
}

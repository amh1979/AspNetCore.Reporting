...
namespace AspNetCore.Reporting.Gauge.WebForms
{
	internal class StateIndicatorCollection : NamedCollection
	{
		...
		private StateIndicator this[string name]
		{
			get
			{
				return (StateIndicator)base.GetByNameCheck(name);
			}
			set
			{
		...
		protected override void OnInsertComplete(int index, object value)
		{
			...
			if (stateIndicator.Position.DefaultValues && index != 0)
			{
				...
				stateIndicator.Location.X = (float)(stateIndicator2.Location.X + 3.0);
				stateIndicator.Location.Y = (float)(stateIndicator2.Location.Y + 3.0);
			}

...
namespace AspNetCore.Reporting.Gauge.WebForms
{
	internal class XamlRenderer : IDisposable
	{
		...
		public void ParseXaml(RectangleF viewportRect, Color[] layerHues, int? maxLayers = default(int?))
		{
			...
			int num = maxLayers.HasValue ? maxLayers.Value : this.CountChildNodes(xmlNode);
		...
		{
			if (this.LayerHues != null && layerIndex < this.LayerHues.Length && !(this.LayerHues[layerIndex] == Color.Transparent))
			{

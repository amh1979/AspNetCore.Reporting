...
namespace AspNetCore.Reporting.Map.WebForms
{
	internal class PathRuleConverter : CollectionItemTypeConverter
	{
		...
		{
			base.simpleType = typeof(PathRule);
		}
		...
		public override PropertyDescriptorCollection GetProperties(ITypeDescriptorContext context, object value, Attribute[] attributes)
		{
			...
			for (int i = 0; i < properties.Count; i++)
			{
				if (properties[i].IsBrowsable)
				{
					...
					{
						if ((properties[i].Name == "FromColor" || properties[i].Name == "MiddleColor" || properties[i].Name == "ToColor") && pathRule.ColoringMode == ColoringMode.DistinctColors)
						{
							propertyDescriptorCollection.Add(TypeDescriptor.CreateProperty(value.GetType(), properties[i], new ReadOnlyAttribute(true)));
							continue;
						}
						if (properties[i].Name == "CustomColors" && !pathRule.UseCustomColors)
						{

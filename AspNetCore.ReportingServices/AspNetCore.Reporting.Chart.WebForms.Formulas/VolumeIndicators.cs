	...
	internal class VolumeIndicators : PriceIndicators
	{
		...
		public override void Formula(string formulaName, double[][] inputValues, out double[][] outputValues, string[] parameterList, string[] extraParameterList, out string[][] outLabels)
		{
			...
			try
			{
				...
				{
				case "MONEYFLOW":
					this.MoneyFlow(inputValues, out outputValues, parameterList, extraParameterList);
					break;
				case "ONBALANCEVOLUME":
					this.OnBalanceVolume(inputValues, out outputValues, parameterList, extraParameterList);
					break;
				case "NEGATIVEVOLUMEINDEX":
					this.NegativeVolumeIndex(inputValues, out outputValues, parameterList, extraParameterList);
					break;
				case "POSITIVEVOLUMEINDEX":
					this.PositiveVolumeIndex(inputValues, out outputValues, parameterList, extraParameterList);
					break;
				case "PRICEVOLUMETREND":
					this.PriceVolumeTrend(inputValues, out outputValues, parameterList, extraParameterList);
					break;
				case "ACCUMULATIONDISTRIBUTION":
					this.AccumulationDistribution(inputValues, out outputValues, parameterList, extraParameterList);
					break;
				default:
					outputValues = null;
					break;
		...
		private void MoneyFlow(double[][] inputValues, out double[][] outputValues, string[] parameterList, string[] extraParameterList)
		{
			...
			for (int k = num2 - 1; k < inputValues[1].Length; k++)
			{
				num3 = 0.0;
				num4 = 0.0;
				for (int l = k - num2 + 1; l <= k; l++)
				{
					num4 += array4[l];
					num3 += array3[l];
				}
				outputValues[0][k - num2 + 1] = inputValues[0][k];
				outputValues[1][k - num2 + 1] = 100.0 - 100.0 / (1.0 + num3 / num4);
			}
		}

	...
	internal class SunkenBorder : IBorderType
	{
		...
		public virtual float Resolution
		{
			set
			{
				...
				this.defaultRadiusSize = (float)(15.0 * this.resolution / 96.0);
				this.cornerRadius = new float[8]
				{
					this.defaultRadiusSize,
					this.defaultRadiusSize,
					this.defaultRadiusSize,
					this.defaultRadiusSize,
					this.defaultRadiusSize,
		...
		private void DrawScrew(ChartGraphics graph, RectangleF rect)
		{
			...
			graph.DrawLine(pen, (float)(rect.X + 2.0 * this.resolution / 96.0), (float)(rect.Y + rect.Height - 2.0 * this.resolution / 96.0), (float)(rect.Right - 2.0 * this.resolution / 96.0), (float)(rect.Y + 2.0 * this.resolution / 96.0));
			pen = new Pen(Color.FromArgb(128, Color.Black), 1f);
			graph.DrawEllipse(pen, (float)(rect.X + 1.0 * this.resolution / 96.0), (float)(rect.Y + 1.0 * this.resolution / 96.0), rect.Width, rect.Height);
			graph.DrawLine(pen, (float)(rect.X + 3.0 * this.resolution / 96.0), (float)(rect.Y + rect.Height - 1.0 * this.resolution / 96.0), (float)(rect.Right - 1.0 * this.resolution / 96.0), (float)(rect.Y + 3.0 * this.resolution / 96.0));
		}
	}
}
